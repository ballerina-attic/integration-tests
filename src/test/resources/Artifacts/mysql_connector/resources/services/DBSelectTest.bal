...
function selectWithLimit () (json, errors:Error){
    ...
    try {
        datatable dt = connectorInstance.select ("select CEIL(count(CustomerID)*50/100) as countTenPercent from Customers", parameters);
        ResultCount rs;
        ...
        parameters = [para];
        dt = connectorInstance.select ("select CustomerName from Customers ORDER BY TotalPurchases DESC limit 4", parameters);
        data, _ = <json>dt;
    } catch (errors:Error e) {
        err = e;
    }
